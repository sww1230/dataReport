export default {
  ...
  listener(area, newVal,oldVal, row, allInfo, $set){
    console.log(area, newVal,oldVal, row, allInfo, $set)
    if(newVal.query.data.radio != oldVal.query.data.radio){
      row.content.data.page.currentPage = 1
      row.content.data.query.data.value = 33
    }
  },
  fetch:{
    aa(row){
  ...
  content: {
    data: {
      ...
      column: [{          // 表格头的配置数据
        ...
        field: 'address',
        label: '地址',
        template: `<el-button @click='submit(scope.row)'  style="padding:0 10px;" round>{{scope.row.name}}</el-button>`

        // template: (row)=>{
        //   return `<button  size="mini" round>${row.address}</button>`
        // }
      }],
      tableData: [{       // 表格内容数据
        date: '2016-05-02',
